#!/usr/bin/env python3
"""
delta_calc_ternary.py  —  Dispersion-energy dip analysis for TERNARY compounds
-------------------------------------------------------------------------------
Reads aggregated *_output.csv files produced by collect_ternary_candidates.py.
Those files live in:
    mp_pipeline_results/aggregated/Sc-Ti-O_output.csv   (example)

Identical dip-detection logic to measure_binary_dips.py, just handles the
3-part chemsys tag (M1-M2-L) correctly everywhere.

Dip detection — two strategies:
...
# ============================================================
#  PATHS  — same root as collect_ternary_candidates.py
# ============================================================
ROOT     = Path(__file__).resolve().parent
AGG_DIR  = ROOT / "aggregated"
OUT_DIR  = ROOT / "delta_analysis_ternary"
...
SLOPE_SIGMA        = 2.0    # std-deviations below mean gradient (fallback)
RIGHT_FLAT_PCTILE  = 25     # percentile of |dy| on right side treated as "flat"
RIGHT_MIN_OFFSET   = 2      # minimum points past dip before right anchor search
...
LOCAL_MAX_WINDOW   = 15     # points after a peak to search for the dip minimum
...
def detect_dip(x: np.ndarray, y: np.ndarray):
    ...
    Strategy 1 — Local max then drop:
        Find every local maximum, search next LOCAL_MAX_WINDOW points
        for the deepest drop exceeding MIN_DIP_DROP.
        Handles curves with strong rising/falling backgrounds.

    Strategy 2 — Global gradient threshold (fallback):
        Points where dy << mean(dy) - SLOPE_SIGMA*std(dy).
        Works for simple bowl-shaped curves.
    """
    debug = {}
    n     = len(y)
    dy    = np.gradient(y, x)
...
def estimate_baseline(x: np.ndarray, y: np.ndarray, dip_idx: int, peak_idx: int = None):
    ...
    Left anchor:
        If peak_idx known (Strategy 1) → use it directly (true shoulder).
        Otherwise walk left for gradient shoulder, fallback to fraction.

    Right anchor:
        Last point in flattest quartile of |dy| to the right of the dip.
...
def tag_from_stem(stem: str) -> str:
    """
    Convert filename stem like 'Sc-Ti-O_output' → 'Sc-Ti-O'.
    Works for both binary (Sc-O) and ternary (Sc-Ti-O) stems.
    """
...
def process_file(csv_path: Path):
    chemsys = tag_from_stem(csv_path.stem)

    # Quick guard: only process ternary files (3 dash-separated parts)
    parts = chemsys.split("-")
    if len(parts) != 3:
        print(f"  [SKIP] {csv_path.name} — not a ternary chemsys ({chemsys}), skipping.")
        return
    ...
    for source, grp in df.groupby("source"):
        ...
        # parse M1, M2, ligand from chemsys tag
        m1, m2, ligand = parts[0], parts[1], parts[2]
        ...
        try:
            ...
            # safe filename: replace any remaining special chars
            safe_source = source.replace("/", "_").replace("\\", "_")
            out_plot = PLOT_DIR / f"{chemsys}_{safe_source}.png"
...
def main():
    ...
    # filter to ternary only (3 dash-separated parts before _output)
    ternary_files = [
        f for f in files
        if len(tag_from_stem(f.stem).split("-")) == 3
    ]
    binary_files  = len(files) - len(ternary_files)

    print(f"Found {len(files)} total aggregated file(s) in {AGG_DIR}")
    print(f"  Ternary (M1-M2-L) : {len(ternary_files)}")
    print(f"  Binary  (skipped) : {binary_files}\n")

    if not ternary_files:
        print("[ERROR] No ternary files found. "
              "Make sure collect_ternary_candidates.py has completed and produced M1-M2-L_output.csv files.")
        return

    all_result_rows = []

    for f in ternary_files:
        process_file(f)

        chemsys   = tag_from_stem(f.stem)
    ...
    if all_result_rows:
        ...
        print(f"  Unique ternary chemsys     : {combined['chemsys'].nunique()}")
        ...
        # ligand breakdown
        print(f"\n  Accepted dips by ligand:")
        print(combined["ligand"].value_counts().to_string())

        # top 20 largest |ΔE|
        print(f"\n  Top 20 largest |ΔE|:")
        top = (combined
               .reindex(combined["delta_E"].abs().sort_values(ascending=False).index)
               .head(20)[["chemsys", "source", "dip_cell_length", "delta_E"]]
               .to_string(index=False))
        print(top)

        combined_out = OUT_DIR / "all_ternary_deltaE_combined.csv"
        combined.to_csv(combined_out, index=False)
        print(f"\n  Combined CSV → {combined_out}")
    else:
        print("\n  No accepted dips across all ternary files.")
