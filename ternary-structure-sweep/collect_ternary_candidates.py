#!/usr/bin/env python3
"""
Ternary batch pipeline for fetching Materials Project data and generating delta plots.
"""
...
from runtime_settings import env_bool, env_csv_list, env_float, env_int, get_api_key, get_dftd3_command
...
# If True, include same-metal pairs like Sc-Sc. Default False (only distinct unordered pairs).
ALLOW_IDENTICAL_METAL_PAIRS = env_bool("ALLOW_IDENTICAL_METAL_PAIRS", False)
...
# Outer: number of concurrent chemsys tasks (metal_pair×ligand)
...
def fetch_chemsys_write_csv(chemsys: str, api_key: str) -> Path:
    ...
    chemsys should be like "Sc-Ti-O" (order doesn't matter for MP, but we choose a canonical ordering)
    """
    safe_name = sanitize_filename(chemsys)
    out_csv = CSV_DIR / f"{safe_name}_table_export_with_structures.csv"
...
def parse_output_for_props(
    ...
    # Dispersion energy
    for ln in lines:
        m = re.search(r"Dispersion\s+energy[:\s]+([-.\dEe+]+)", ln, re.I)
        if m:
            Edis = m.group(1)
            break
...
def process_single_poscar(poscar_file: Path, chemsys: str, n_workers: int) -> List[dict]:
    ...
    safe_chemsys = sanitize_filename(chemsys)
    target_dir = STRAIN_DIR / safe_chemsys / base_name
...
def run_strain_and_collect(poscar_dir: Path, chemsys: str) -> Path:
    ...
    """
    safe_chemsys = sanitize_filename(chemsys)
    agg_csv = AGG_DIR / f"{safe_chemsys}_output.csv"
...
def plot_aggregated_csv(agg_csv: Path, chemsys: str, y_min: float = -0.6, y_max: float = 0.0):
    ...
    out_png = PLOT_DIR / f"{sanitize_filename(chemsys)}_cell_length_vs_Edis.png"
...
def process_chemsys(m1: str, m2: str, ligand: str, api_key: str):
    """
    Process a single chemsys defined by two metals and a ligand, e.g. "Sc-Ti-O".
    """
    # canonical ordering: sort metals so M1-M2 == M2-M1
    metals_sorted = sorted([m1, m2], key=lambda s: s.upper())
    chemsys = f"{metals_sorted[0]}-{metals_sorted[1]}-{ligand}"
    chemsys_tag = sanitize_filename(chemsys)
...
def main():
    ...
    print("\n==============================")
    print("TERNARY PIPELINE CONFIG")
    print("==============================")

    # Build unordered metal pairs
    if ALLOW_IDENTICAL_METAL_PAIRS:
        metal_pairs = list(itertools.combinations_with_replacement(D_BLOCK_METALS, 2))
    else:
        metal_pairs = list(itertools.combinations(D_BLOCK_METALS, 2))

    total_tasks = len(metal_pairs) * len(LIGANDS)

    print(f"Metals (d-block): {len(D_BLOCK_METALS)}")
    print(f"Ligands: {LIGANDS}")
    print(f"Allow identical metal pairs: {ALLOW_IDENTICAL_METAL_PAIRS}")
    print(f"Total chemsys tasks: {total_tasks}")
    ...
    # Build task list (m1,m2,ligand) with canonical unordered metal pairs
    tasks = []
    for (m1, m2) in metal_pairs:
        for lig in LIGANDS:
            tasks.append((m1, m2, lig))

    # Run tasks in parallel
    results = []
    with ProcessPoolExecutor(max_workers=N_TASK_WORKERS) as executor:
        futures = {executor.submit(process_chemsys, m1, m2, lig, api_key): (m1, m2, lig) for (m1, m2, lig) in tasks}

        for future in as_completed(futures):
            m1, m2, lig = futures[future]
            try:
                results.append(future.result())
            except Exception as e:
                chemsys = f"{m1}-{m2}-{lig}"
